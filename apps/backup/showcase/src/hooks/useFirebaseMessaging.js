...
if (Platform.OS !== 'web') {
    try {
        ...
        if (__DEV__) {
            console.log('[FIREBASE] Successfully imported messaging module');
        }
    }
    catch (e) {
        if (__DEV__) {
...
export function useFirebaseMessaging() {
    ...
    const foregroundHandlerRef = { current: null };
    /**
    ...
    const requestPermission = useCallback(async () => {
        try {
            ...
            if (!messaging || Platform.OS === 'web') {
                if (__DEV__) {
                    console.log('[FIREBASE] requestPermission() - Firebase not available or on web');
                }
                return false;
            }
            if (__DEV__) {
    ...
    const getToken = useCallback(async () => {
        try {
            ...
            if (!messaging || Platform.OS === 'web') {
                if (__DEV__) {
                    console.log('[FIREBASE] getToken() called on web - no-op');
                }
                return null;
            }
            if (__DEV__) {
    ...
    const enable = useCallback(async () => {
        ...
        try {
            ...
            if (__DEV__) {
                console.error('[FIREBASE] Enable error:', errorMsg);
            }
            setError(errorMsg);
            return null;
        }
     ...
     */
    const setupForegroundListener = useCallback((handler) => {
    ...
    const disable = useCallback(async () => {
        try {
            ...
            if (__DEV__) {
                console.error('[FIREBASE] Disable error:', err);
            }
        }
    }, []);
...
export function setupBackgroundMessageHandler() {
    ...
    try {
        ...
        if (__DEV__) {
            console.error('[FIREBASE] Failed to setup BG handler:', e);
        }
    }
}
