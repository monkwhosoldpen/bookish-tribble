...
function MenubarSubTrigger({
  ...
  const iconName = Platform.OS === 'web' ? 'chevron-right' : open ? 'keyboard-arrow-up' : 'keyboard-arrow-down';
  return (
    <TextClassContext.Provider
      value={cn(
      ...
      <MenubarPrimitive.SubTrigger
        ...
        {...props}>
        <>{children}</>
        <Icon as={MaterialIcons} name={iconName} className={cn('text-foreground ml-auto size-4 shrink-0', iconClassName)} />
      </MenubarPrimitive.SubTrigger>
    </TextClassContext.Provider>
  );
}
